...
- `gzip-file-as-resource`: Max fetch size controlled by `GZIP_MAX_FETCH_SIZE` (default 10MB), timeout by `GZIP_MAX_FETCH_TIME_MILLIS` (default 30s), remote URLs restricted to HTTPS domains in `GZIP_ALLOWED_DOMAINS`
